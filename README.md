
<!-- Animated Header -->
<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:0e75b6,100:8a2be2&height=220&section=header&text=Priyanshu%20Poonia&fontSize=45&fontColor=ffffff&animation=fadeIn&fontAlignY=35"/>

<h3 align="center">
C++ Developer • DSA • Low-Level Systems • Graphics Programming
</h3>

<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=22&pause=1000&color=0E75B6&center=true&vCenter=true&width=800&lines=Building+Strong+Foundations+in+C%2B%2B+%F0%9F%94%A5;Learning+DSA+and+Problem+Solving+Daily+%E2%9C%85;Exploring+Memory%2C+Rendering+and+Engine+Architecture+%F0%9F%9A%80;Building+Projects+from+Scratch+%F0%9F%92%BB;Focused+on+Long-Term+Engineering+Growth"/>
</p>

<p align="center">
  <a href="https://github.com/11parasf">
    <img src="https://komarev.com/ghpvc/?username=11parasf&label=Profile%20Views&color=0e75b6&style=for-the-badge" alt="profile views"/>
  </a>

  <a href="https://github.com/11parasf?tab=followers">
    <img src="https://img.shields.io/github/followers/11parasf?label=Followers&style=for-the-badge&color=8a2be2"/>
  </a>
</p>

---

# 👋 About Me

Hi, I'm **Priyanshu Poonia**.

I'm currently focused on mastering:

- **C++**
- **Data Structures & Algorithms**
- **Low-Level Programming Concepts**
- **Graphics / Engine Development**
- **Web Development**

My long-term goal is to become a strong software engineer capable of building complex systems from scratch — not just using frameworks blindly.

Currently learning deeply about:
- OOP & STL
- Memory management
- Problem solving
- Linux & developer tools
- Rendering concepts
- Engine architecture
- Clean project structure

---

# 🛠️ Tech Stack

<p align="center">
  <img src="https://skillicons.dev/icons?i=cpp,c,python,html,css,js,nodejs,git,github,linux,vscode,figma&perline=6" />
</p>

---

# 🚀 Current Projects

### ✅ Inventory Management System
C++ based project focused on logic building, OOP concepts and clean structure.

### ✅ iPhone Style Calculator
Building a modern calculator UI using C++ with graphics libraries like Raylib / SFML.

### ✅ DSA Repository
Structured DSA solutions with optimized approaches and explanations.

### 🚧 Physics Engine
Learning vectors, collision systems, transformations and simulation fundamentals.

### 🚧 Portfolio Website
Designed in Figma and developed for showcasing projects, notes and learning journey.

---

# 📚 Current Focus

- Advanced C++
- STL & Problem Solving
- DSA Practice
- Linux
- OpenGL & Graphics Programming
- Memory / Allocator Concepts
- System Design Fundamentals
- Building Real Projects

---

# 🏆 GitHub Trophies

<p align="center">
  <img src="https://github-profile-trophy.vercel.app/?username=11parasf&theme=tokyonight&no-frame=true&row=1&column=7"/>
</p>

---

# 📊 GitHub Stats

<p align="center">
  <img height="170em" src="https://github-readme-stats.vercel.app/api?username=11parasf&show_icons=true&theme=tokyonight&hide_border=true"/>

  <img height="170em" src="https://github-readme-streak-stats.herokuapp.com/?user=11parasf&theme=tokyonight&hide_border=true"/>
</p>

<p align="center">
  <img height="170em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=11parasf&layout=compact&theme=tokyonight&hide_border=true"/>
</p>

---

# 📈 Contribution Graph

<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=11parasf&theme=tokyo-night&hide_border=true"/>
</p>

---

# ⚡ Philosophy

```cpp
while (!success)
{
    learn();
    build();
    breakThings();
    improve();
}
````

I believe strong engineering skills come from:

* building projects
* understanding fundamentals
* debugging deeply
* consistency over shortcuts

---

# 🔗 Connect With Me

<p align="center">
  <a href="https://github.com/11parasf">
    <img src="https://img.shields.io/badge/GitHub-111111?style=for-the-badge&logo=github&logoColor=white"/>
  </a>

  <a href="https://www.linkedin.com/in/pooniapriyanshu9">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>
</p>

---

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:8a2be2,100:0e75b6&height=120&section=footer"/>

